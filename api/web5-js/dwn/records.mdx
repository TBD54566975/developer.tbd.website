...
`delete(id)`:  Generates a _delete_ entry tombstone for the record. This is a convenience method that allows you to easily delete records

`update()`: Takes in a new request object matching the expected method signature of a _write_ and overwrites the record
