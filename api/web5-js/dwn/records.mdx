...
## Overview
The `records` class is responsible for providing convenience methods for CRUD operations interfacing with `dwn`s.
...
`recordId` - The unique identifier based on the record entry's composition. Note: all entries across all records are deterministically unique.

`descriptor` - the descriptor object for the constructed `dwn` message.

`data` - An object with the following methods available on it:
