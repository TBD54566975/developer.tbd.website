...
The `web5` instance is our single entry point for all Web5 operations. You've also set up a `todos` array to hold our ToDos, and a variable to remember our decentralized identifier (DID) as `did`, which you'll create or load in a moment.
...
onBeforeMount(async () => {
  ...
  // We'll put more code in later sections here
});
```
...
```js
  // ... Below code from "onBeforeMount" from above
...
Now that your todos are stored in your app, you can display them by using your `todos` object. In this example, you can do it by adding the following HTML code to the Vue app below the JS, just as you would in any non-web5 app. In our template there is a comment marked `<!-- Todos -->` where this code block fits nicely:
...
To allow for add functionality, begin by creating the UI needed to add todos. In our template there is a section marked `<!-- Add Todo Form -->` where this block may go:
...
async function addTodo() {
  ...
  // More code from later sections will go here
}
```

This code uses the `myDid` object we loaded or created in the `onBeforeMount` function and uses it to write into the DWN that `web5` is using to store our data.

Once we’ve written the data to storage, we’ll use the returned `record` to create our app's `todo` object, passing along the `recordId` :

```js
  // Add this below the code from "addTodo" above
...
Deleting todos can be done by using the `delete` method, passing the `recordId`:
...
We’ll connect that to our UI with the following code. This replaces the `<!-- Delete Todo goes here later -->` comment we placed from above:
