...
const BreadcrumbTab = ({ snippetMappings, dependencies }) => {
  const [activeTab, setActiveTab] = useState(
    snippetMappings
      ? Object.keys(snippetMappings)[0]
      : dependencies[0].props.language,
  );
  ...
  return (
    <div>
      <div className="breadcrumbTabs" style={{ display: "flex" }}>
        {(snippetMappings
          ? Object.keys(snippetMappings)
          : dependencies.map((dep) => dep.props.language)
        ).map((title) => (
          <div
            key={title}
      ...
      <div>
        {activeTab &&
          ...
          dependencies.map((dep) => {
            return (
              <div
                key={dep.props.language}
                style={{
                  display: activeTab === dep.props.language ? "block" : "none",
                }}
              >
                {dep}
              </div>
