...
const allCredentials = [signedEmploymentVcJwt, signedNameAndDobVcJwt];
...
describe('Presentation Exchange Process', () => {
  ...
  test('selectCredentials() selects VCs that match presentation defintion', async () => {
    const selectedCredentials = await pex_selectCredentials(allCredentials, pd);
  ...
  test('satisfiesPresentationDefinition() checks if VCs satisfy PD', async () => {
    const selectedCredentials = await pex_selectCredentials(allCredentials, pd);
    const satisfied = await pex_checkPresentationDefinitionSatisfaction(selectedCredentials, pd);
    expect(satisfied).toBe(true);
  });

  test('createPresentationFromCredentials() creates a presentation result', async () => {
    const presentationResult = await pex_createPresentationFromCredentials(allCredentials, pd);
  ...
  test('validateSubmission() checks if the presentation submission is valid', async () => {
    const pr = await pex_createPresentationFromCredentials(allCredentials, pd);
