        ...
        value: <ApiDetails
                ...
                },
                {
                    data: {
                        ...
                        type: 'string',
                        field: 'sync',
                        value: "Enable or disable synchronization of DWN records between local and remote DWNs. Sync defaults to running every 2 minutes and can be set to any value accepted by `ms`. To disable sync set to `'off'",
                    },
                },
                {
                    data: {
...
##### Specify list of decentralized web nodes endpoints to define
```js
...
##### Instantiate Web5 instance with a Web5Agent and existing DID to connect to.
```js
const web5 = new Web5({
  agent: testAgent.agent,
  connectedDid: myExistingDid
});
```
</section>
