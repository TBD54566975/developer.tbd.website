...
- **Credential Request**: The steps needed for your Wallet App to request a KCC **post** successful IDV, on a users behalf.
- **Credential Management**: How the Wallet utilizes existing credentials for KCC requests and secure storage of the KCC itself.

:::info
For a detailed guide on Known Customer Credentials for **Issuers**, please refer to our [Issuer's Known Customer Credential Guide](/)
:::
...
Your Wallet will require libraries for [handling HTTP request](https://www.npmjs.com/package/node-fetch), [managing credentials](https://www.npmjs.com/package/@web5/credentials) and [processing JSON Web Tokens(JWTs)](https://www.npmjs.com/package/@web5/credentials), which you can include in your project by installing the following dependencies:

```bash
npm install @web5/credentials @web5/dids node-fetch
```
...
IDV is a critical component of typical KYC requirements, where certain types of Personally Identifiable Information (PII) are collected and verified from an individual. IDV is a crucial step before a KCC can be issued.

[**Wallet's IDV Flow diagram**]
...
1. Resolve the Issuer's DID to get the IDV service Endpoint that can be found in the Issuer's [DID Document](/docs/web5/learn/decentralized-identifiers#did-documents):
...
2. Send an HTTP request to the IDV Service Endpoint:
...
- `vp_token` is a Verifiable Presentation that contains certified and authenticated claims about the customer. In this guide, we'll cover an example `vp_token` and `id_token`.
...
3. Decode the Issuer's SIOPv2 Authorization Request and create a [SIOPv2 Authorization Response](https://openid.github.io/SIOPv2/openid-connect-self-issued-v2-wg-draft.html#section-10) with `id_token` and `vp_token`:
...
If `vp_token` wasn't in the SIOPv2 Response, then the Issuer will respond with a `credential_offer` and a `url` that points to a webpage for the customer to input their PII (e.g.; name, identification number, date of birth, etc). After successful completion of this form, the Wallet can proceed with the next steps, [processing the Credential Offer](https://hackmd.io/SQwE3SnrRkmeipGGtznJTw?both#Processing-the-Credential-Offer).

[**GIF of PII info being filled out?**]
...
1. Extract and store the `credential_issuer` URL and `pre_authorized_code` from the credential offer:
...
2. Fetch the Issuer's Metadata from the `credential_issuers` URL
...
3. Fetch the Authorization Server's Metadata

Before the Wallet can request a KCC from the Issuer. In the same `handleIssuerResponse()` the Wallet needs to also fetch the Issuer's Authorization Server Metadata. Similarly, by appending `/.well-known/oauth-authorization-server` to the `credential_issuer` URL:
...
4. Fetch the Access Token from the Issuer's `/token` endpoint:
...
1. Sign and Send an HTTP POST request to the Issuer's credential endpoint:
...
The Known Customer Credential will be a signed VC [JSON Web Token](https://jwt.io/) for secure transmission.
:::
...
```javascript
{
  "vcDataModel": {
    ...
    "issuer": "did:dht:z6Mkn4w6nSaWe4fjNJRvaHZwFnMm5VexvjzDeozEu2G7jC34", // Issuer's DID
    "issuanceDate": "2024-01-01T19:23:24Z",
    "expirationDate": "2026-05-19T08:02:04Z"
    "credentialSubject": {
      "id": "did:dht:z6MkjGSeekPGE9QfczHWyW8v2ZzJU68kqSHzV7L2dmQyuyDu", // User's DID
