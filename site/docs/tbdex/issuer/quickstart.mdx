...
Within your service's entry point file (e.g. index.js), import the following:
...
To start up your Express API service, add the following to a `main.ts` file:
...
```typescript title=SanctionsCredential.ts
import { VerifiableCredential } from "@web5/credentials";
