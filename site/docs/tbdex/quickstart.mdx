...
✅  Use a wallet app to send tbDEX messages to a PFI to facilitate an exchange
...
VCs are stored as JSON Web Tokens (JWTs). Your wallet app has saved Alice's new VC into an array named `myCredentials`. Let's now find which of Alice’s credentials meet the PFI's requirements:
...
<WalletQuickstartCodeRunner executeCodeName="executeCreateOrder" stepIndex={6} customPlaceholder={"Create your order"} viewJsonObj={true} collapseLevel={1}/> 
...
<WalletQuickstartCodeRunner executeCodeName="executeSendOrder" stepIndex={7} customPlaceholder={"Submit your order"} viewJsonObj={true} collapseLevel={1}/> 
