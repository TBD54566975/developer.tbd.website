...
The Offering we selected has a `requiredClaims` section that specifies [Verifiable Credentials](/docs/web5/learn/verifiable-credentials) (VCs) Alice needs  in order to be eligible for the Offering. This PFI requires Alice to have a `SanctionsCredential`, as outlined in the `requiredClaims` section of the Offering:
