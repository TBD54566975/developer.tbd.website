...
The Offering we selected has a `requiredClaims` section that specifies [Verifiable Credentials](/docs/web5/verifiable-credentials/what-are-vcs) (VCs) Alice needs in order to be eligible for the Offering. This PFI requires Alice to have a `SanctionsCredential`, as outlined in the `requiredClaims` section of the Offering. Additionally, that credential must be issued by a specific issuer as described by the issuer's DID below:
