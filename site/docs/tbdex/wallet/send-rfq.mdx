...
* **payinSubunits** - Amount of payin currency the customer wants in exchange for payout currency
...
## Send RFQ to PFI
Use `TbdexHttpClient` to send the RFQ to the PFI, specifying the message type as `rfq`:
...
Your application will poll this exchange until a message of kind `quote` appears in the exchange:
...
while (!quote) {
  ...
  const exchange = exchanges.data[0];
  quote = exchange.find(msg => msg.kind === 'quote');
...
Now that you've received a Quote, you can present this to your user and give them the option to place an Order.
