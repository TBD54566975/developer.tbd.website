...
Given you have a PFI's DID, you can query their DID document to get their most recent service endpoint URL.

```js
const foundService = pfiDid.document.service.find(service => service.type === 'PFI');
const serviceEndpoint = foundService.serviceEndpoint;
```
