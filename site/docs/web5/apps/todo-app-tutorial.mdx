...
<div className="grid grid-cols-1 desktop:grid-cols-1">
  <iframe className="aspect-video" src="https://www.youtube.com/embed/2wAVyId6QHc" title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen></iframe>
</div>
...
```jsx
<div v-if="(todos.length > 0)" className="border-gray-200 border-t border-x mt-16 rounded-lg shadow-md sm:max-w-xl sm:mx-auto sm:w-full">
  <div v-for="todo in todos" :key="todo" className="border-b border-gray-200 flex items-center p-4">
    <div @click="toggleTodoComplete(todo)" className="cursor-pointer">
      <CheckCircleIcon className="h-8 text-gray-200 w-8" :className="{ 'text-green-500': todo.data.completed }" />
    </div>
    <div className="font-light ml-3 text-gray-500 text-xl">
...
```jsx
<div className="mt-16">
  <form className="flex space-x-4" @submit.prevent="addTodo">
    <div className="border-b border-gray-200 sm:w-full">
      <label for="add-todo" className="sr-only">Add a todo</label>
      <textarea
        rows="1" name="add-todo" id="add-todo" v-model="newTodoDescription"
        @keydown.enter.exact.prevent="addTodo"
        className="block border-0 border-transparent focus:ring-0 p-0 pb-2 resize-none sm:text-sm w-96"
        placeholder="Add a Todo" />
    </div>
    <button type="submit" className="bg-indigo-600 border border-transparent focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 hover:bg-indigo-700 inline-flex items-center p-1 rounded-full shadow-sm text-white">
...
<div className="ml-auto">
  <div @click="deleteTodo(todo)" className="cursor-pointer">
    <TrashIcon className="h-8 text-gray-200 w-8" :className="'text-red-500'" />
  </div>
</div>
```
