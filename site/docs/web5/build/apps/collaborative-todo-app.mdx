...
This tutorial takes it a step further. We will expand on the core features introduced in the todo app to make it more collaborative - enabling multiple users to interact with a shared todo list.
...
You can follow along with this tutorial using your own UI layout, or you can use the starter project. As you go through the tutorial, you'll add logic to the methods and update the UI.
...
Copy the following block and paste under all `import` statements:
...
Let's call the `Web5.connect()` function, which will create a new DID and DWN for them if they don't already have these components.
...
const configureProtocol = async () => {
    ...
    // instantly send protocol to remote DWNs as well
...
- First we query the list of existing protocols to check if our protocol already exists on the DWN.
- If the protocol already exists, we return and do nothing.
- If the protocol does not exist, we set it up on the local DWN and send it to the remote DWN.
...
We can add the following variable declarations:
...
### Fetch List Details
In our dynamic todo page, let's import `Web5` and the protocol definition document again so they can be reused.

```js title="pages/todos/_id.vue"
import { Web5 } from "@web5/api";
import sharedTodoProtocol from "~/assets/shared-todo-protocol.json";
```

We need to fetch the list details from the DWN so let's get the list id from the url parameter, connect to Web5 and use the listId to query the DWN for the list.
...
To get the DID of the recipient to send the record to, since either of the author or recipient of the list could be the one creating the todo, we use a `getTodoRecipient()` to help us out.
