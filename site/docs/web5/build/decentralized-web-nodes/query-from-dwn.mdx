...
## Querying Records by Path and Type
The following snippets demonstrates how to query records by path and type to match a defined protocol. The protocol definition below has an audio type and a video type:

<CodeSnippet functionName="queryFromDwnDefinedProtocol" />

Based on the the protocol definition above, you can use the the following snippet to query for audio records by path and type:

<CodeSnippet functionName="queryFromDwnByPathAndType" />
