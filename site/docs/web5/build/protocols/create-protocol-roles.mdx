...
Actors with these roles can be assigned various permissions like read, write, query, update, or delete operations on the data stored in a [Decentralized Web Node (DWN)](https://developer.tbd.website/docs/web5/learn/decentralized-web-nodes/).
...
- **$contextRole**: used to assign permissions that are relevant only within a specific context or specific part of the system. For example, a moderator role for a particular forum within a larger platform could be a context role. These roles are defined at the second level of the protocol structure.
