...
For the **JavaScript SDK**, a successful validation would result in

```js
[ Checked { tag: 'root', status: 'info', message: 'ok' } ]
```
 For the **Kotlin SDK**, this returns an undefined result, but if unsuccessful, it will throw an error. 
