...
If your application accepts verifiable credentials, you'll want to define what sort of information is necessary to meet the requirements. This specification of expected claims is a **Presentation Definition** (PD).
...
:::info
For more in-depth information on the schema and options, refer to the [Presentation Defintion spec](https://identity.foundation/presentation-exchange/spec/v1.0.0/#json-schema-2).
:::
...
:::info
The `filter` property offers a lot of flexibility. Refer to the [Presentation Defintion spec](https://identity.foundation/presentation-exchange/spec/v1.0.0/#json-schema-2) for more examples.
:::
...
The `rule` is a required property of `submission_requirements` and must be "all" or "pick" to indicate if all input descriptors in the Presentation must be satisfied, or if the applicant is allowed to pick a subset to satisfy. 
...
```js
{
  "vc": {
    ...
    "issuanceDate": "2023-01-05T16:06:00Z",
    "credentialSubject": {
      "id": "did:example:alice",
      "certifications": [
        {
