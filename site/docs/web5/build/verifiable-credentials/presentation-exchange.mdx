...
This method will throw an error if the VCs do not satisfy the Presentation Definition.
...
The `validateSubmission()` method returns an array of `Checked` objects representing the validation checks performed on the presentation submission:
...
Once the Presentation is error-free and has passed the validation checks, you can submit the Verifiable Presentation by providing the lender with the `presentation` JSON object:
