...
[ Checked { tag: 'root', status: 'info', message: 'ok' } ]
    ...
    {
      content: "If unsuccessful, it will throw an error. ",
      language: 'Kotlin', 
    }
  ]}
/>
...
<Shnip
   snippets={[
    ...
    { 
      content: 'After creating and validating the [Presentation](/docs/glossary#verifiable-presentation) with `VerifiablePresentation.create()`, you can submit a signed [Verifiable Presentation](/docs/glossary#verifiable-presentation) encoded as a JWT, so the lender can [verify](/docs/web5/build/verifiable-credentials/verify-vc#verify-verifiable-presentations) it. To submit a signed Verifiable Presentation, the Holder signs it with their DID:', 
