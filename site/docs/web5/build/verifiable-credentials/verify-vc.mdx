---
title: Verify VCs and VPs
hide_title: true
sidebar_position: 2
---

import verifyVcs from '!!raw-loader!@site/snippets/testsuite-javascript/__tests__/web5/build/verifiable-credentials/verifyVcs.snippet.js'
import errorsFound from '!!raw-loader!@site/snippets/testsuite-javascript/__tests__/web5/build/verifiable-credentials/errorsFound.snippet.js'
import evaluatePresentation from '!!raw-loader!@site/snippets/testsuite-javascript/__tests__/web5/build/verifiable-credentials/evaluatePresentation.snippet.js'

import verifyVcsKt from '!!raw-loader!@site/snippets/testsuite-kotlin/src/test/kotlin/docs/web5/build/verifiablecredentials/verifyVcsKt.snippet.kt'
import errorsFoundKt from '!!raw-loader!@site/snippets/testsuite-kotlin/src/test/kotlin/docs/web5/build/verifiablecredentials/errorsFoundKt.snippet.kt'
import signVpKt from '!!raw-loader!@site/snippets/testsuite-kotlin/src/test/kotlin/docs/web5/build/verifiablecredentials/signVpKt.snippet.kt'
import verifyVpKt from '!!raw-loader!@site/snippets/testsuite-kotlin/src/test/kotlin/docs/web5/build/verifiablecredentials/verifyVpKt.snippet.kt'
...
<Shnip
  snippets={[
    { snippetContent: verifyVcs, language: 'JavaScript'},
    { snippetContent: verifyVcsKt, language: 'Kotlin'}
    ]}
/>
...
<Shnip
  snippets={[
    { snippetContent: errorsFound, language: 'JavaScript'},
    { snippetContent: errorsFoundKt, language: 'Kotlin'}
    ]}
/>
...
Aggregating one or more VCs into a VP provides the following benefits:
- **Convenience:** Instead of presenting several VCs individually, a VP enables an entity to present all relevant credentials packaged together.
- **Privacy:** Rather than sharing the entire credential, entities can selectively share specific information from each credential to protect personal data.
- **Contextual Relevance:** VPs include metadata that explain the purpose and relevance of each credential to verifiers.
- **Adaptability:** An entity can customize a VP for different situations to demonstrate various aspects of their background in one presentation. 
For example, in a job application, a VP can highlight an applicant's educational qualifications, work experience, and skills.

### Sign VP with DID

The first step is to encode a Verifiable Presentation into a JWT by signing it with the holder's DID. The **holder**, who signs and presents the VP, 
could be the subject of the credentials or an entity authorized to represent the subject.
...
<Shnip
  ...
  snippets={[
    { snippetContent: signVpKt, language: 'Kotlin'},
  ]}
/>

### Verify VP
After the Verifiable Presentation is encoded as a JWT, the VP can be verified via the `VerifiablePresentation.verify()` function.

This function will:

- **Credential Verification:** Confirms that each VC inside the VP is valid and legitimate.
- **Presentation Authenticity:** Confirms the VP is unaltered from its original form and validates the legitimacy of both the issuer and the presenter.
...
<Shnip
  ...
  snippets={[
    { snippetContent: verifyVpKt, language: 'Kotlin'},
  ]}
/>
...
<Shnip
  snippets={[
    { snippetContent: evaluatePresentation, language: 'JavaScript'},
  ]}
    inlineSnippets={[
    {
