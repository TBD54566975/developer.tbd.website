...
The `web5 vc create` command can create a Verifiable Credential without signing it.

```bash
web5 vc create <credential-subject-did-uri> --issuer <issuer-did-uri> [options]
```
**Options**
...
# Create an unsigned VC with an issuer and an expiration date
web5 vc create did:dht:kfnps1cwyt6dmb5g1q8z9xncx4nmepz4pn6cqck3jswhpg9bp4qo --issuer did:dht:p364ofunt9wu6u8jrt44ka6uz588iuefbnwjhxgbm8fjbmki4fco --expiration-date 2025-12-31
```

## Create and Sign a VC
The `web5 vc create` command can create and sign a VC, converting it into a [JWT](/docs/web5/verifiable-credentials/jwt-to-vc/#what-is-a-jwt).
...
**Options**
* `--portable-did <did>`: Specifies the DID used to sign the Verifiable Credential and outputs it as a JWT. If included, the Portable DID's URI is automatically set as the issuer.
* `--issuer <issuer-did-uri>`: Specifies the issuer of the credential.
* `--expiration-date <date>`: Set an expiration date for the credential.
* `--no-indent`: Output the VC without indentation.
* `--json-escape`: Escape JSON strings in the output.
