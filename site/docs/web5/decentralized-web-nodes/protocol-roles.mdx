...
That sentence works well! It’s clear, concise, and effectively communicates the process of defining custom roles. Here’s your final version:

To define a custom role in your protocol, define the role as an element in the `types` section. 
Use the `structure` object to outline their permissions by marking the `$role` as `true` and assigning `$actions` to specify what the role can do.
