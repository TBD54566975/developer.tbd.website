...
A [DID](/docs/web5/learn/decentralized-identifiers) essentially acts as a URI that associates the subject of the DID (the person, company, or object being identified) with a DID document that lives off-chain in a decentralized storage system such as IPFS.
...
DID Documents describe how to interact with the DID subject and contains information that allows others to verify the authenticity and integrity of the DID's information. The document includes things like the DID subject's public keys, authentication and verification methods, as well as service endpoints, such as [DWNs](/docs/web5/learn/decentralized-web-nodes), that reference the locations of the subject’s data.

## How to Retrieve a DID Document
Given a person attempts to use their DID within an app to identify themselves, the app would _resolve_ their DID to find their mini-profile (the DID document).

To do this in Web5, you'd call the `web5.did.resolve()` function:
...
The `resolve` function will return a `DidResolutionResult`, which contains the `DIDocument` object:
