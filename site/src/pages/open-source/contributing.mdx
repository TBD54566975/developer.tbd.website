...
1. First you'll need to create your test. Navigate to `site/__test__` this is where you'll create your test file. Your test file should be named to match the funtionality of the code.
   Here's an example test file naming convention: `how-to-create-did.test.js`

:::note
The `.test.` naming convention. This is how Vite will know to run these tests.
:::

2. In your test files, use the `:snippet-start:` and `:snippet-end:` tags to denote the beginning and end of a code snippet. Name your snippet immediately after the `:snippet-start:` tag. For example:
...
To use the snippet in your documentation(`.mdx` file), you're going to use the `Shnip` component in the following format:

```mdx
// if it's a multi language doc, you'll need to specify it like this at the top of the file;

<LanguageSwitcher languages="JavaScript, Kotlin" />

<Shnip
  snippets={[{ name: 'createDidDht', language: 'JavaScript' }]}
  snippets={[{ name: 'kt_createDidDht', language: 'Kotlin' }]} // if you have another generated snippet in a different language
/>
```
