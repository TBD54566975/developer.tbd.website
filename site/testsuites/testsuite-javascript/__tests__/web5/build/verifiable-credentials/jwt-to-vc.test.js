import { test, expect, describe } from 'vitest';
import {
  assignVcJwt,
  parseSignedVcJwt,
} from '../../../../../../code-snippets/web5/build/verifiable-credentials/jwt-to-vc';

const jwt =
  'eyJ0eXAiOiJKV1QiLCJhbGciOiJFZERTQSIsImtpZCI6ImRpZDpkaHQ6a2ZkdGJjbTl6Z29jZjVtYXRmOWZ4dG5uZmZoaHp4YzdtZ2J3cjRrM3gzcXppYXVjcHA0eSMwIn0..ntcgPOdXOatULWo-q6gkuhKmi5X3bzCONQY38t_rsC1hVhvvdAtmiz-ccoLIYUkjECRHIxO_UZbOKgn0EETBCA';

describe('decode JWT into VC', () => {
  test('jwt string is correct', async () => {
    const vc_jwt = await assignVcJwt();
    expect(vc_jwt).toBe(jwt);
  });

  test('VerifiableCredential.parseJwt() returns a VC', async () => {
    const vc = await parseSignedVcJwt(jwt);
    expect(vc).toBeDefined();
    expect.soft(vc).toHaveProperty('type', 'EmploymentCredential');
    expect
      .soft(vc)
      .toHaveProperty(
        'issuer',
        'did:dht:kfdtbcm9zgocf5matf9fxtnnffhhzxc7mgbwr4k3x3qziaucpp4y',
      );
    expect
      .soft(vc)
      .toHaveProperty(
        'subject',
        'did:dht:y39m48ozoete7z6fzaangcos87fhuh1dzi7f7jwbjft7otskh9to',
      );
    expect
      .soft(vc.vcDataModel)
      .toHaveProperty('id', 'urn:uuid:8fd52030-1caf-4978-a35d-d417eb8e00b2');
    expect
      .soft(vc.vcDataModel)
      .toHaveProperty('expirationDate', '2022-09-30T12:34:56Z');
    expect
      .soft(vc.vcDataModel.credentialSubject)
      .toHaveProperty(
        'id',
        'did:dht:y39m48ozoete7z6fzaangcos87fhuh1dzi7f7jwbjft7otskh9to',
      );
    expect
      .soft(vc.vcDataModel.credentialSubject)
      .toHaveProperty('position', 'Software Developer');
    expect
      .soft(vc.vcDataModel.credentialSubject)
      .toHaveProperty('startDate', '2021-04-01T12:34:56Z');
    expect
      .soft(vc.vcDataModel.credentialSubject)
      .toHaveProperty('employmentStatus', 'Contractor');
  });
});
